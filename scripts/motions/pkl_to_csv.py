...
def main(
    ...
    print(f"FPS: {fps}")
    print(f"Original duration: {original_duration:.2f} seconds ({frames.shape[0]} frames)")

    # Repeat motion if duration is specified.
    if duration is not None:
        if duration < original_duration:
            print(f"Warning: Requested duration ({duration}s) is shorter than original ({original_duration:.2f}s)")
            ...
            print(f"XY displacement per cycle: [{displacement_per_cycle[0]:.4f}, {displacement_per_cycle[1]:.4f}]")
        ...
        print(f"New duration: {frames.shape[0] / fps:.2f} seconds ({frames.shape[0]} frames)")
    ...
    if add_start_transition:
        ...
        print(f"Creating {transition_duration}s start transition ({transition_frames} frames)...")
        ...
        print(
            f"  First frame orientation: yaw={np.degrees(yaw):.1f}°, pitch={np.degrees(euler_zyx[1]):.1f}°,"
            f" roll={np.degrees(euler_zyx[2]):.1f}°"
        )
        print(f"  Start orientation: yaw={np.degrees(yaw):.1f}° (matched), pitch=0°, roll=0°")
    ...
    if add_end_transition:
        ...
        # Number of transition frames.
        transition_frames = int(transition_duration * fps)
        print(f"Creating {transition_duration}s end transition ({transition_frames} frames)...")
        ...
        print(
            f"  Final frame orientation: yaw={np.degrees(yaw):.1f}°, pitch={np.degrees(euler_zyx[1]):.1f}°,"
            f" roll={np.degrees(euler_zyx[2]):.1f}°"
        )
    ...
    if pad_duration > 0:
        ...
        print(f"Total duration: {frames.shape[0] / fps:.2f} seconds ({frames.shape[0]} frames)")
    ...
    print(f"Final CSV shape: {csv_data.shape} (columns: 3 pos + 4 quat_xyzw + {joint_dof.shape[1]} joints)")
